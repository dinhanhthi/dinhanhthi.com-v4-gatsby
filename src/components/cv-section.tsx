...
export default function CVSection({
  ...
  return (
    <div
      className={`${
        className ? className : ''
      } flex flex-wrap items-center text-main dark:text-main-dark`}
    >
      <div className={'hidden w-1/4 flex-col items-center py-4 px-2 md:flex'}>
        ...
        <div className="mt-1 text-sm opacity-70">{cv.date}</div>
      </div>
      <div className="flex-1 px-4 py-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <h4 className="text-base md:hidden">{cv.where}</h4>
            <div className="mt-1 text-sm opacity-70 md:hidden">{cv.date}</div>
          </div>
        </div>
        <div className="mt-2 flex flex-col gap-2">
          {cv.activity.map((activity: string) => (
            <div className="opacity-95 dark:opacity-80">{parse(activity)}</div>
          ))}
        </div>
        {cv.tech && (
